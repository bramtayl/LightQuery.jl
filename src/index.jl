...
function getindex(mapped::Generator, index)
    mapped.f(getindex(parent(mapped), index))
end
function getindex(filtered::Filter, index)
    getindex(parent(filtered), index)
end
function view(mapped::Generator, indexes)
    Generator(mapped.f, view(parent(mapped), indexes))
end
function view(filtered::Filter, indexes)
